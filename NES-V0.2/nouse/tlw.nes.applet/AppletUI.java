...
import tlw.nes.HiResTimer;
import tlw.nes.KbInputHandler;
import tlw.nes.NES;
import tlw.nes.intf.IBufferView;
import tlw.nes.intf.InputHandler;
import tlw.nes.intf.UI;
...
public class AppletUI implements UI{
	...
	public void imageReady(boolean skipFrame){
		...
		if(Globals.enableSound && Globals.timeEmulation && tmp>0){
			...
			while(nes.getPapu().getLine().available() < min_avail){
				timer.yield();
			}
