...
public class Mapper001 extends MapperDefault{
		...
		if(reg == 0){
			...
			if(tmp != mirroring){
				...
				if((mirroring & 2)==0){
					...
					nes.getPpu().setMirroring(ROM.MIRRORING_SINGLE);
				}else{
					// Not overridden by SingleScreen mirroring.
					////System.out.println("MMC1: Setting Normal Mirroring. value="+mirroring);
					nes.getPpu().setMirroring((mirroring&1)!=0?ROM.MIRRORING_HORIZONTAL:ROM.MIRRORING_VERTICAL);
				}
			}
	...
	protected void switchLowHighPrgRom(int oldSetting){

		// not yet.

	}

	protected void switch16to32(){

		// not yet.

	}

	protected void switch32to16(){
